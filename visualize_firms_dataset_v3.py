...
def _enforce_requested_window(
    ...
    if dropped:
        ...
        print("           A source returning out-of-window rows is serving a "
              "different period than asked for. Note that archive-only mode "
              "is NOT the remedy: it drops NOAA-21 entirely (see --use-archive).")
...
def fetch_firms_data(
    ...
    if df.empty and use_sample_fallback:
        ...
        if start_date or end_date:
            print("  No FIRMS data for requested date range. FIRMS serves historical "
                  "dates on NRT sources too, so keep all sources enabled rather than "
                  "narrowing to --use-archive.")
        else:
...
EE_RASTER_LAYER_CONFIG = {
    ...
    "population": ("CIESIN/GPWv411/GPW_Population_Density", ["population_density"], False, "Population density"),  # pragma: allowlist secret
}
...
# ---------------------------------------------------------------------------
# The Earth Engine Cloud project every API call is billed and quota-counted
# against. Resolution order: --ee-project, then $EE_PROJECT, then this default.
# Keeping a default here means a forgotten env var falls back to the right
# project rather than silently re-using whichever one was last authenticated.
DEFAULT_EE_PROJECT = "ee-lmu-ndp-wildfire"


def resolve_ee_project(explicit: str | None = None) -> str:
    """The EE project to use, in precedence order."""
    return (explicit or os.environ.get("EE_PROJECT", "").strip()
...
def _init_earth_engine(project: str | None = None) -> bool:
    ...
    if not HAS_EARTH_ENGINE:
        return False
    proj = resolve_ee_project(project).strip()
    with _EE_INIT_LOCK:
        ...
        try:
            ...
            if "no project found" in err_msg:
                ...
                print(f"  Current project: {proj or '(none)'}")
                print("  Provide --ee-project or enter it when prompted:")
                print(f"    setx EE_PROJECT {DEFAULT_EE_PROJECT}      (Windows, then reopen the terminal)")
                print(f"    export EE_PROJECT={DEFAULT_EE_PROJECT}    (macOS/Linux)")
...
def run_interactive() -> argparse.Namespace:
    ...
    print("   5 = Look up a fire by NAME (nationwide; sets the bbox and dates")
    print("       from the published perimeter)")
    ...
    if area_choice == "all":
        ...
        for a in area_choice.replace(" ", "").split(","):
            if a == "2":
                ...
                # Name lookup sets bbox AND dates, so it runs before the date
                # prompts are used and overrides the defaults chosen above.
                args.fire_name = _prompt("Fire name")
                year_raw = _prompt("Year (blank = any)", "").strip()
                args.fire_year = int(year_raw) if year_raw.isdigit() else None
                args.fire_state = _prompt("State (blank = anywhere)", "").strip() or None
                args.fire_pad_km = float(_prompt("Pad the perimeter by (km)", "5") or 5)
                args.fire_lead_in_days = int(_prompt("Lead-in days", "2") or 2)
                args.fire_tail_days = int(_prompt("Tail days", "3") or 3)
                args.bbox = None
                args.start_date = args.end_date = None
                if resolve_fire_name_args(args):
                    args.areas.append("custom")
                else:
                    print("  falling back to California")
    ...
    print("   Auto is right for historical pulls too: NOAA-21 has no SP product,")
    print("   so 'Archive only' drops that satellite entirely.")
    ...
    if args.enrich_earth_engine or args.ee_raster_layers:
        ...
        args.ee_project = _prompt(
            "GCP/EE project ID", resolve_ee_project()
        ).strip() or None
...
def resolve_fire_name_args(args) -> bool:
    """Fill in --bbox and the date range from a named fire.

    Turns `--fire-name "Bootleg" --fire-state OR` into the bbox and window
    this pipeline already understands, so nothing downstream changes. Returns
    False if the name could not be resolved.

    Explicit flags win: a --bbox or --start-date the user typed is never
    overwritten, because silently ignoring what someone asked for is worse
    ...
    chain = (fl.NATIONAL_CHAIN if getattr(args, "fire_national", False)
             else fl.SOURCE_CHAIN)
    print(f"Resolving fire '{args.fire_name}'"
          + (f" ({args.fire_year})" if args.fire_year else "")
          + (f" in {args.fire_state}" if args.fire_state else "") + "...")
    hits = fl.resolve_fire(args.fire_name, args.fire_year,
                           args.fire_state, chain)
    if not hits:
        print(f"  no fire named '{args.fire_name}' found. Tried: "
              + ", ".join(chain))
        return False

    rec = hits[0]
    if len(hits) > 1:
        print(f"  {len(hits)} matches; using the largest:")
        for other in hits[1:4]:
            print(f"    also: {other.describe().splitlines()[0]}")
    print(f"  {rec.describe().splitlines()[0]}")
    if rec.dates_uncertain:
        print("  WARNING: this record carries only a fire year, so the fetch "
              "window starts at January 1.\n           Set --start-date and "
              "--end-date yourself if you know the real dates.")
    if not rec.exact_footprint:
        print("  WARNING: no perimeter published; extent inferred from acreage.")

    if args.bbox:
        print("  keeping the --bbox you supplied")
    elif rec.bbox:
        w, s_, e, n = fl.pad_bbox(rec.bbox, args.fire_pad_km)
        args.bbox = f"{w:.4f},{s_:.4f},{e:.4f},{n:.4f}"
        args.areas = ["custom"]
        print(f"  bbox {args.bbox}  (+{args.fire_pad_km:g} km)")
    else:
        print("  that record has no geometry; cannot derive a bbox")
        return False

    if args.start_date or args.end_date:
        print("  keeping the dates you supplied")
    else:
        start, end = rec.date_window(args.fire_lead_in_days, args.fire_tail_days)
        if start:
            args.start_date, args.end_date = start, end
            print(f"  dates {start} .. {end}  "
                  f"({args.fire_lead_in_days}d lead-in, "
                  f"{args.fire_tail_days}d tail)")
    return True
...
def main() -> None:
    ...
    parser.add_argument("--use-archive", action="store_true",
                        help="Use archive/SP sources only. WARNING: FIRMS publishes no "
                             "VIIRS_NOAA21_SP product, so this silently drops all NOAA-21 "
                             "detections (~20%% of a typical CA pull). NRT sources serve "
                             "historical dates fine; prefer the default for older data.")
    ...
    parser.add_argument("--fire-name", type=str, default=None, metavar="NAME",
                        help="Resolve the area and dates from a named fire "
                             "(NIFC/WFIGS/CAL FIRE) instead of --bbox. "
                             "Works nationally.")
    parser.add_argument("--fire-year", type=int, default=None,
                        help="Narrow --fire-name to one year")
    parser.add_argument("--fire-state", type=str, default=None,
                        help="Narrow --fire-name to a state, e.g. CA, OR, MT")
    parser.add_argument("--fire-national", action="store_true",
                        help="Search NIFC/WFIGS first. Default is CA-first "
                             "(FRAP), which has better ignition dates.")
    parser.add_argument("--fire-pad-km", type=float, default=5.0,
                        help="Pad the resolved perimeter by this many km "
                             "(default 5)")
    parser.add_argument("--fire-lead-in-days", type=int, default=2,
                        help="Extra days fetched before ignition (default 2)")
    parser.add_argument("--fire-tail-days", type=int, default=3,
                        help="Extra days fetched after containment (default 3)")
    ...
    parser.add_argument("--ee-project", type=str, default=None,
                        help=f"GCP/EE project ID. Falls back to $EE_PROJECT, then "
                             f"'{DEFAULT_EE_PROJECT}'.")
    ...
    # Resolve a named fire into bbox + dates before anything else reads them.
    if getattr(args, "fire_name", None) and not args.interactive:
        if not resolve_fire_name_args(args):
            raise SystemExit(1)
